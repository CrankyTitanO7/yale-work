...
LinkedList *zip_list(LinkedList *list_a, LinkedList *list_b)
{
    ...
    if (current1 == NULL) // if a is empty do copy
    {
        ...
        current2 != NULL)
    {
        ...
        if (next1 != NULL) {
        current2->next = next1;  // only redirect if list_a has more nodes
    }
        // next1->next  = next2;
    ...
    // Your function would still work for current input file even though you don't handle this edge case.
    // However, some test will fail if you don't handle this edge case on Gradescope.
    // It is a good practice to handle edge cases.
    ...
    // else if (current1->next != NULL && current2->next == NULL) current2->next = current1;
