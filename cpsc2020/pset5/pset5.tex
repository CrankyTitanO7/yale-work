...
\begin{outline}[enumerate]
    ...
    \1 Prove that if you have k  cents postage for some $k \geq 14$ then you must have either 1) at least 2 4-cent stamps; 2) at least 2 9-cent stamps; or 3) at least 1 15-cent stamp.
        ...
        base case: a,b,c = 0; then k < 14 so cannot be true
            ...
            \3 cases where 1 of a,b,c are 0 but two are not.
                ...
                \4 a,b $\neq$ 0;$k = 4a + 9b$ if $a \leq 1\wedge b$ $\leq 1$, then k $<$ 14. By contrapositive (modus ponens), a,b must be $\geq 2$ each
    ...
    \1 Find the smallest k so that you can make k' cents postage for any $k' \geq k$ with 4-, 9-, and 15-cent stamps.
        ...
        \2 
        strong induction: for all natural numbers p(i) > 19, if p(n) is true for n = 18, … , k - 1
        then p(k) is true
            \3 base case: is p(n) true for 18? yes, because 18 = 9 $\cdot$ 2 = 2b
            \3 base case: is p(n) true for 19? yes, because 19 = 15 + 4 = c + a
            \3 base case: is p(n) true for 20? yes, because 20 = 4 $\cdot 5$= 5a
    ...
    (continued from proof above)
        \2 strong induction: for all natural numbers p(i) > 19, if p(n) is true for n = 18, … , k - 1
        then p(k) is true
        
            \3 suppose p(i) is true for all i where $18 \leq i \leq k-1$
            p(k) = 4a + 9b + 15c

            since $a \geq 2 \vee b \geq 2 \vee c \geq 1$ for P(i), p(k) is true 
\end{outline}
